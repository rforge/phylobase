...
class NxsCharactersBlock
  : public NxsCharactersBlockAPI, public NxsTaxaBlockSurrogate
	...
	public:
		...
		std::vector<double>		GetContinuousValues(unsigned i, unsigned j, const std::string key);
		...
		double					GetSimpleContinuousValue(unsigned i, unsigned j);
		...
		bool					IsInterleave();
		bool					IsEliminated(unsigned charIndex);
		...
		void					SetSupportMixedDatatype(bool v) {supportMixedDatatype = v;}

		virtual void			DebugShowMatrix(std::ostream &out, bool use_matchchar, char *marginText = NULL) const;
...
class NxsDiscreteDatatypeMapper
	...
	public:
		...
		std::string		 StateCodeToNexusString(int) const;
		...
		void			 WriteStateCodeAsNexusString(std::ostream & out, int scode) const;
		bool			 WasRestrictionDataype() const;
...
*/
inline std::string NxsDiscreteDatatypeMapper::StateCodeToNexusString(int scode) const
	{
	std::ostringstream o;
	WriteStateCodeAsNexusString(o, scode);
	return o.str();
	}
...
inline std::vector<double> NxsCharactersBlock::GetContinuousValues(
	...
	const std::string key) /* The name of the ITEM in the FORMAT command.  Must be ALL CAPS.*/
	{
...
*/
inline const char *NxsCharactersBlock::GetSymbols()
	{
...
inline	unsigned NxsCharactersBlock::CharLabelToNumber(NxsString s)
	{
...
inline bool NxsCharactersBlock::IsEliminated(
  unsigned charIndex)	/* the character in question */
	{
...
inline void NxsDiscreteDatatypeMapper::WriteStateCodeRowAsNexus(std::ostream & out, std::vector<int>::const_iterator & begIt, const std::vector<int>::const_iterator & endIt) const
	...
	for (; begIt != endIt; ++begIt)
		WriteStateCodeAsNexusString(out, *begIt);
	}
