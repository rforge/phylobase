...
BlockReaderList NxsReader::parseFileWithReader(
	...
	nexusReader.RemoveFactory(&factory);
	BlockReaderList brl = nexusReader.GetUsedBlocksInOrder();
	return brl;
    }
...
{
	BlockReaderList blocks = GetUsedBlocksInOrder();
...
void PublicNexusReader::Add(NxsBlock *)
{
	assert(false);
}
...
NxsStoreTokensBlockReader * PublicNexusReader::GetUnknownBlock(unsigned index) const
	{
	return storerBlockVec.at(index);
	}
