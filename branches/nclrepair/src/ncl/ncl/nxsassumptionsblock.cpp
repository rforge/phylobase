...
void NxsAssumptionsBlock::HandleTypeSet(
  NxsToken &token)	/* the token used to read from in */
	...
	assert(cbp);
	effectiveAssumpBlock->ReadPartitionDef(newPartition, *cbp, typeset_name, "Character", "TypeSet", token, false);
...
void NxsAssumptionsBlock::HandleCodeSet(
  NxsToken &token)	/* the token used to read from in */
	...
	NxsCharactersBlockAPI *cbp = effectiveAssumpBlock->GetCharBlockPtr();
	assert(cbp);
	effectiveAssumpBlock->ReadPartitionDef(newPartition, *cbp, codeset_name, "Character", "CodeSet", token, false);
...
void NxsAssumptionsBlock::HandleCodonPosSet(
  NxsToken &token)	/* the token used to read from in */
	...
	NxsCharactersBlockAPI *cbp = effectiveAssumpBlock->GetCharBlockPtr();
	assert(cbp);
	effectiveAssumpBlock->ReadPartitionDef(newPartition, *cbp, codonPosSetName, "Character", "CodonPosSet", token, false);
	for (NxsPartition::const_iterator groupIt = newPartition.begin(); groupIt != newPartition.end(); ++groupIt)
		{
		const std::string & s = groupIt->first;
		bool legal = false;
...
void NxsAssumptionsBlock::HandleWeightSet(
  NxsToken &token)	/* the token used to read from in */
	...
	if (isVect)
		...
		effectiveAssumpBlock->ReadPartitionDef(newPartition, *cbp, wtset_name, "Character", "WtSet", token, false);
	bool floatWts = false;
	long currLongWt;
	double currDblWt;
	NxsTransformationManager &ctm = cbp->GetNxsTransformationManagerRef();
...
void NxsAssumptionsBlock::HandleCharPartition(
  NxsToken &token)	/* the token used to read from in */
	...
	NxsCharactersBlockAPI *cbp = effectiveAssumpBlock->GetCharBlockPtr();
	assert(cbp);
	effectiveAssumpBlock->ReadPartitionDef(newPartition, *cbp, charpart_name, "Character", "CharPartition", token, asterisked);
...
void NxsAssumptionsBlock::HandleTaxPartition(
  NxsToken &token)	/* the token used to read from in */
	...
	NxsPartition newPartition;
	assert(taxa);
	effectiveAssumpBlock->ReadPartitionDef(newPartition, *taxa, taxpart_name, "Taxa", "TaxPartition", token, asterisked);
...
void NxsAssumptionsBlock::HandleTreePartition(
  NxsToken &token)	/* the token used to read from in */
	...
	effectiveAssumpBlock->ReadPartitionDef(newPartition, *treesBlockPtr, treepart_name, "Tree", "TreePartition", token, asterisked);
	effectiveAssumpBlock->AddTreePartition(treepart_name, newPartition);
	}

void NxsBlock::ReadPartitionDef(NxsPartition &np, NxsLabelToIndicesMapper &ltm, const std::string & partName, const char * ptype, const char * cmd, NxsToken & token, bool warnAsterisked, bool demandAllInds)
	{
