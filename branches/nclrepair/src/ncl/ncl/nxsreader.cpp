...
NxsBlock *FindBlockByTitle(const BlockReaderList & chosenBlockList, const char *title, unsigned *nMatches)
	...
	for (BlockReaderList::const_iterator cblIt = chosenBlockList.begin(); cblIt != chosenBlockList.end(); ++cblIt)
		{
		const std::string n = (*cblIt)->GetTitle();
		const bool isMatch = ((emptyTitle && n.empty()) || (NxsString::case_insensitive_equals(title, n.c_str())));
		if (isMatch) 
			{
			if (toReturn)
				*nMatches = *nMatches + 1;
...
*/
char *NxsReader::NCLCopyrightNotice()
	{
...
*/
char *NxsReader::NCLNameAndVersion()
	{
